...
for (.file in survey_inequality_filelist) {
  ...
  data_output <- data %>% select(-MODEL, -SCENARIO, -UNIT, -str_subset(names(data), "V[0-9]")) %>% rename(iso3=REGION) %>% mutate(dist=ifelse(str_detect(VARIABLE, "Inequality Index"), "0", paste0("D", parse_number(gsub("-","",VARIABLE[VARIABLE!="Inequality Index|Gini" & VARIABLE!="Inequality Index|Absolute Poverty" & VARIABLE!="Inequality Index|Gini_exp"])))))
